    ...
    {{-- Toggle HT/TTC (TTC = HT * 1.20) --}}
        (function() {
            ...
            function refresh() {
                ...
                {{-- Afficher/Masquer les colonnes HT/TTC dans le simulateur --}}
                document.querySelectorAll('.colht').forEach(el => {
                    el.style.display = isTTC ? 'none' : '';
                });
                document.querySelectorAll('.colttc').forEach(el => {
                    el.style.display = isTTC ? '' : 'none';
                });
            }
            if (toggle) {
            ...
            {{-- Prix et validités chargés depuis le .env via config/pricing.php --}}
            const PRICES = window.ticketConfig.prices;
            const VALIDITIES = window.ticketConfig.validities;
