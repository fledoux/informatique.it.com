...
@section('t                {{-- Boutons de réponse --}}
                ...
                @foreach($ticket->messages()->orderBy('created_at', 'desc')->get() as $message)
                    @if($message->status === 'active' || (auth()->user()->hasAnyRole(['admin', 'super-admin', 'manager']) && $message->status === 'internal'))
                        <div class="row">
                            <div class="col-2 mb-3 text-end">
                                <span class="fw-bold {{ $message->status === 'internal' ? 'text-warning' : ($message->author_id === $ticket->author_id ? 'text-secondary' : 'text-primary') }}">
                                    ...
                                    @if($message->status === 'internal')
                                        <i class="fa-solid fa-lock text-warning" title="Message interne"></i>
                                    @endif
                                </span><br>
                            ...
                            <div class="col-12 col-lg-10 mb-3">
                                <div class="card border {{ $message->status === 'internal' ? 'border-warning border-2' : ($message->author_id === $ticket->author_id ? 'border-2' : 'border-primary border-2') }}">
                                    <div class="card-body">
                                        @if($message->subject !== 'Re: ' . $ticket->subject)
                ...
                <div class="row mt-4 pt-3 border-top">
                    <div class="col-12">
                        <h6 class="text-muted mb-3"><i class="fa-regular fa-clock me-2"></i>Message original</h6>
                    </div>
                    <div class="col-2 mb-3 text-end">
