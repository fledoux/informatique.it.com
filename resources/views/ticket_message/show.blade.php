...
@section('title', __('global.Details') . '  ' . __('ticket_message.entity'))

@section('content')
    <h1 class="h3 mb-3">{!! __('global.Details') !!}  {{ __('ticket_message.entity') }}</h1>

    <dl class="row">
        <dt class="col-sm-3">{{ __('ticket_message.id') }}</dt>
        <dd class="col-sm-9">{{ $ticketMessage->id }}</dd>
        <dt class="col-sm-3">{{ __('ticket_message.fields.status') }}</dt>
        <dd class="col-sm-9">{{ $ticketMessage->status ?? '' }}</dd>
        <dt class="col-sm-3">{{ __('ticket_message.fields.subject') }}</dt>
        <dd class="col-sm-9">{{ $ticketMessage->subject ?? '' }}</dd>
        <dt class="col-sm-3">{{ __('ticket_message.fields.body') }}</dt>
        <dd class="col-sm-9">{{ $ticketMessage->body ?? '' }}</dd>
        <dt class="col-sm-3">{{ __('ticket_message.fields.company_id') }}</dt>
        <dd class="col-sm-9">{{ $ticketMessage->company_id ? \App\Models\Company::find($ticketMessage->company_id)?->name : '' }}</dd>
        <dt class="col-sm-3">{{ __('ticket_message.fields.ticket_id') }}</dt>
        <dd class="col-sm-9">{{ $ticketMessage->ticket_id ? \App\Models\Ticket::find($ticketMessage->ticket_id)?->public_uuid : '' }}</dd>
        <dt class="col-sm-3">{{ __('ticket_message.fields.author_id') }}</dt>
    ...
    <div class="btn-group mt-3" role="group" aria-label="Actions">
        <a href="{{ route('ticket_message.edit', $ticketMessage) }}" class="btn btn-primary">{!! __('global.Edit') !!}</a>
